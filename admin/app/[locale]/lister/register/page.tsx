...
export default function ListerRegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    const { data, error: err } = await supabase.auth.signUp({
      ...
      options: { data: { full_name: form.full_name, role: 'lister' } }
    })
    ...
    <div className="min-h-screen bg-[#0e0e1a] flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <div className="text-xs text-indigo-400 font-bold tracking-widest uppercase">{t('title')}</div>
        </div>
        ...
        <p className="text-center text-xs text-gray-500 mt-4">
          ...
          <Link href={`/${locale}/login`} className="text-indigo-400 hover:underline">{t('sign_in')}</Link>
        </p>
      </div>
    </div>
  )
