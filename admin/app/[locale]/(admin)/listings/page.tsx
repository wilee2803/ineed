...
export default async function ListingsPage({
  ...
  const tabs = [
    { key: 'pending_review', label: t('tab_review') },
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6 border-b border-gray-200">
        {tabs.map(tab => (
          <Link
            key={tab.key}
            href={`/${locale}/listings?status=${tab.key}`}
          ...
          {listings.map((l: any) => (
            <div key={l.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500">
                    <span>💶 € {Number(l.price).toLocaleString(locale)}{l.listing_type === 'rent' ? '/Mo' : ''}</span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {l.status === 'pending_review' && (
                    <>
                      <Link
                        href={`/${locale}/listings/${l.id}?action=approve`}
                      ...
                      <Link
                        href={`/${locale}/listings/${l.id}?action=reject`}
              ...
              <div className="mt-3 pt-3 border-t border-gray-50 flex gap-4 text-xs text-gray-400">
                <span>Erstellt: {new Date(l.created_at).toLocaleDateString(locale)}</span>
